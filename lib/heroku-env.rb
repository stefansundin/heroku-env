module HerokuEnv
  ...
  def run
    ...
    # MySQL
    ENV["MYSQL_URL"] ||= ENV["CLEARDB_DATABASE_URL"] || ENV["JAWSDB_MARIA_URL"]
    ...
    ENV["SPARKPOST_SMTP_FROM"] ||= "test@#{ENV["SPARKPOST_SANDBOX_DOMAIN"]}" if ENV["SPARKPOST_SANDBOX_DOMAIN"]

    ENV["SMTP_HOST"] ||= ENV["MANDRILL_SMTP_SERVER"] || ENV["SENDGRID_SMTP_SERVER"] || ENV["MAILGUN_SMTP_SERVER"] || ENV["POSTMARK_SMTP_SERVER"] || ENV["SPARKPOST_SMTP_HOST"]
    ENV["SMTP_PORT"] ||= ENV["MAILGUN_SMTP_PORT"] || ENV["SPARKPOST_SMTP_PORT"] || "587"
    ENV["SMTP_USERNAME"] ||= ENV["MANDRILL_USERNAME"] || ENV["SENDGRID_USERNAME"] || ENV["MAILGUN_SMTP_LOGIN"] || ENV["POSTMARK_API_KEY"] || ENV["SPARKPOST_SMTP_USERNAME"]
    ENV["SMTP_PASSWORD"] ||= ENV["MANDRILL_APIKEY"] || ENV["SENDGRID_PASSWORD"] || ENV["MAILGUN_SMTP_PASSWORD"] || ENV["POSTMARK_API_KEY"] || ENV["SPARKPOST_SMTP_PASSWORD"]

    # If you use Postmark, you need to configure MAIL_FROM manually
    ENV["MAIL_FROM"] ||= ENV["MANDRILL_USERNAME"] || ENV["SENDGRID_USERNAME"] || ENV["MAILGUN_SMTP_LOGIN"] || ENV["SPARKPOST_SMTP_FROM"]

    # Airbrake
    ENV["AIRBRAKE_HOST"] ||= "api.airbrake.io" if ENV["AIRBRAKE_API_KEY"]
    ...
    ENV["AIRBRAKE_KEY"] ||= ENV["AIRBRAKE_API_KEY"] || ENV["RAYGUN_APIKEY"] || ENV["ROLLBAR_ACCESS_TOKEN"] || ENV["APPENLIGHT_APIKEY"]
  end
end
